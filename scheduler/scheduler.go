...
func init() {
	...
	broker_customer_info, _ := strconv.ParseUint(os.Getenv("BROKER_CUSTOMER_INFO_RUN_EVERY_SECOND"), 10, 64)
	...
	go func() {
		...
		gocron.Every(broker_customer_info).Second().Do(readCustomerBrokerInfo)
		<-gocron.Start()
	}()
}
